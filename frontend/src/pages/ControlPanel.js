...
function ControlPanel() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      const response = await axios.get(`${API_BASE_PATH}/listings`);
      setListings(
  ...
  const fetchFaveListings = useCallback(async () => {
    try {
      const response = await axios.get(
          `${API_BASE_PATH}/api/favorites/listing/${user.id}`
      );
      setListings(
  ...
  }, [API_BASE_PATH, user.id]); // user.id as a dependency to recreate the function when user.id changes
  ...
  useEffect(() => {
    if (!user) {
      ...
      const checkAdminStatus = async () => {
        try {
          const response = await axios.get(
              `${API_BASE_PATH}/user/${user.id}/is-admin`
          );
  ...
  const toggleListingVisibility = async (id, isVisible) => {
    try {
      await axios.put(
          `${API_BASE_PATH}/listings/${id}/${isVisible ? "show" : "hide"}`
      );
  ...
  const removeFavorite = async (listingId) => {
    try {
      await axios.delete(`${API_BASE_PATH}/api/favorites/delete`, {
        params: {
          userId: user.id,
          listingId: listingId,
        },
